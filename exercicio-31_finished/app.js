...
const nameArr = ["Jor", "ge"];

const nameP = nameArr.map((silaba) => `P${silaba}`).join("");
...
name.split("").forEach((letra, i) => {
  // console.log(`${letra} é a ${++i} letra do meu nome.`);
});
...
const recorrencia = (arr, valorReconrrente) => {
  const recorrente = arr.reduce((acc, valor) => {
    if (valor === valorReconrrente) {
      acc++;
    }
...
// console.log(recorrencia(scores, 60));

...
elementsInsideDiv.forEach((element) => {
  element.addEventListener("click", () => {
    ...
    h2.textContent = `Clicou no ${element.tagName.toUpperCase()} filho da div.`;
  });
});
...
/*
  ...
  - No código acima, faça com que quando um filho da div for clicado, a mensagem exibida no console seja "Clicou no NOME_DA_TAG_COM_LETRAS_MINÚSCULAS, filho
    da div.".
*/
...
/*
  ...
  - Faça com que a mensagem de clique na div e a mensagem de clique em algum  filho da div, ao invés de ser exibida no console, seja inserida neste h2.
*/
...
h2.addEventListener("copy", () => console.log("Texto copiado!"));
...
eggElement.addEventListener("mousemove", () => {
  const x = event.x;
  const y = event.y;

  eggElement.textContent = `Eixo X: ${x} | Eixo Y: ${y}`;
});
...
const hasDevFront = people.some(
  (peopleObj) => peopleObj.profession === "Front-end developer"
);

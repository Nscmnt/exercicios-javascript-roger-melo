...
function asyncronuos(callback) {
  setTimeout(callback, 1000);
}
// asyncronuos(() => console.log("Async?"));
...
const lesserThanFive = (num) => num < 5;

console.log(numbers.filter(lesserThanFive));
...
totalPrice = prices.reduce((acc, price) => (acc += price));
...
let booksBox = {
  ...
  addBook(n) {
    ...
    } else if (n + this.booksIn > this.spaces) {
      const remainingBooks = this.spaces - this.booksIn;
      let book = remainingBooks === 1 ? "livro" : "livros";
      return `Só cabem mais ${remainingBooks} ${book}`;
    } else {
      this.booksIn = n;
      return `${n} livros adicionado`;
    }
  },
};
...
console.log(booksBox.addBook(3));

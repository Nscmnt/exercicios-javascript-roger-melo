...
class Clock {
  ...
  render() {
    ...
    let formattedTime = this.template.template
      .replace("h", hours)
      .replace("m", minutes)
      .replace("s", seconds);
...
textArea.addEventListener("input", countNumbersOfLetters);

function countNumbersOfLetters() {
  let numbersOfLetters = textArea.value.trim().length;
  paragraph.textContent = `Foram digitadas ${numbersOfLetters} letras`;
}

...
const ordenedNames = names.reduce((acc, name) => {
  acc.push(name);
...
const ordenedCharaters = [];

characters.forEach((character) => {
  ordenedCharaters.push(character);
  ordenedCharaters.sort((a, b) => a.id - b.id);
});

// console.log(ordenedCharaters);
...
const ordenedNumbers = numbers.reduce((acc, number) => {
  acc.push(number);
  acc.sort((a, b) => a - b);
...
const reverseAlphabeticalOrdering = people.sort((a, b) => b.localeCompare(a));

// console.log(reverseAlphabeticalOrdering);
...
const peopleAmountDisney = topBrazilmovies
  .filter((movie) => movie.distributedBy === "Disney")
  .reduce((acc, movieDisney) => (acc = movieDisney.peopleAmount), 0);
...
/*
  ...
  - Os cães, ao invés da idade original, devem conter sua "idade humana";
    - Algumas pessoas dizem que 1 ano de um cachorro equivale à 7 anos de uma 
      pessoa. Cientificamente, não é tão simples assim, mas para fins didáticos, se baseie nessa informação para fazer o cálculo.
...
` fora de escopo, devido não estar fazendo o curso`;

...
console.log(
  `Até aqui, o objeto "cat" possui as seguintes propriedades e valores: "name", que recebeu ${
    ...
    cat.color
  }, "bestFriends", que recebeu um array com os itens ${cat.bestFriends[0]} e ${
    cat.bestFriends[1]
...
cat.insertFriend = function (friend) {
  this.bestFriends.push(friend);
};

cat.insertFriend("Ted");
...
cat["addColor"] = function (color) {
  this.colorSecondary = color;
};

cat.addColor("brown");

console.log(cat["colorSecondary"]);
...
function sumAges() {
  const sumAgesPets = cat.age + dog.age;

  return `A soma das idades de ${cat.name} e ${dog.name} é ${sumAgesPets}.`;
}

...
const showAnswer = (event) => {
  ...
  const answerEl = target.nextElementSibling;
...
const carMaker = ({ name, color }) => {
  const car = Object.create(volkswagenProto);
...
// console.log(amarok.logCarInfo(), jetta);
...
const getIndexesOfCharacter = (str, charSearch) => {
  const charIndices = str.split("").reduce((acc, char, index) => {
    if (char === charSearch) {
      acc.push(index);
    }
...
// console.log(getIndexesOfCharacter(aString, "p"));
...
const message = "sou fluênte em JS, construo aplicações web com JS Puro";
const typingEl = document.querySelector('[data-js="typing"]');
const time = 200;
const ArrMessage = message.split("");

let maxLength = ArrMessage.length,
  indice = 0;
...
setInterval(() => {
  ...
  if (ArrMessage[indice] === ",") {
    typingEl.textContent = "";
  } else {
    typingEl.textContent += ArrMessage[indice];
  }

...
/*
  ...
  - Faça o mesmo que o exercício acima pede, mas desta vez, implemente uma **function expression** que retorne o resultado da **divisão** entre esses 2 números.
*/
const div = function (n1 = 1, n2 = 1) {
  return n1 / n2;
};
...
function arrayToUpperCase(arr) {
  const arrayUpperCase = arr.map((str) => str.toUpperCase());
...
function arrNumbersInfo(arr) {
  ...
  console.log(
    `O array "randomNumbers" possui ${arr.length} números, sendo ${positives} positivos e ${negatives} negativos.`
  );
}

// arrNumbersInfo(randomNumbers);
...
const sentence = functions.reduce((acc, word) => (acc += word() + " "), "");
